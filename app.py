...
# ────────────────────────────────────────────────
#                 Logging + تحديث yt-dlp + ffmpeg check
# ────────────────────────────────────────────────
logging.basicConfig(
...
async def download_video(url: str, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    ydl_opts = {
        ...
        'fragment_retries': 10,
        'continuedl': True,
        'force_generic_extractor': True,  # يساعد مع مواقع غير مدعومة جيداً
    }
    ...
    try:
        ...
        if "pixeldrain.com" in url.lower():
            try:
                ...
                if r.status_code == 200:
                    ...
                    raise Exception("فشل التنزيل المباشر")
            except Exception as fallback_err:
                logger.warning(f"Fallback Pixeldrain فشل: {fallback_err}")

        # التنزيل الرئيسي بـ yt-dlp
        ...
        caption = (
            f"🎬 {info.get('title', 'ملف بدون عنوان')}\n"
        ...
        if any(word in error_str for word in ["private", "login required", "restricted", "age"]):
            await progress_msg.edit_text("المحتوى خاص أو يتطلب تسجيل دخول. لا يمكن تحميله بدون cookies.")
        elif "no video" in error_str or "no formats" in error_str or "cannot parse" in error_str:
            await progress_msg.edit_text("لا يمكن استخراج الفيديو (ربما غير مدعوم أو مشكلة مؤقتة). جرب رابط آخر.")
        elif "geo" in error_str or "blocked" in error_str:
            await progress_msg.edit_text("المحتوى مقيد جغرافياً أو محظور في منطقتك.")
        else:
            await progress_msg.edit_text(f"خطأ: {str(e)[:150]}...\nجرب رابط آخر أو انتظر تحديث.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "مرحباً كرار! 👋\n"
        "أرسل أي رابط فيديو أو ملف وسأحاول تحميله لك (يدعم معظم المواقع الشهيرة)\n\n"
        "TikTok • Instagram • YouTube • Facebook • Twitter/X • Threads • Pinterest • Pixeldrain • SoundCloud ...\n\n"
        "جرب الآن 🚀",
...
async def help_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "1. انسخ أي رابط فيديو أو ملف\n"
        "2. ألصقه هنا مباشرة\n"
        "3. انتظر → سيحاول البوت التنزيل (يتعامل مع معظم المواقع)\n\n"
        "ملاحظة: المحتوى الخاص أو المحمي قد لا يعمل بدون cookies.\n"
        "إذا فشل، جرب الرابط في متصفح أو أرسله لي مرة أخرى."
    )
